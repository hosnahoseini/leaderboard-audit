...
from clean_bt_rank.experiments.ci_reduction import run_ci_reduction_experiment
...
def test_top_alpha_selection_expands_forward_reverse_pairs() -> None:
    ...
    assert selected["match_id"].tolist() == [1, 1]
...
def test_ci_reduction_influence_uses_fixed_initial_ranking() -> None:
    df = pd.DataFrame(
        [
            {"model_a": "alpha", "model_b": "beta", "winner": "model_a"},
            {"model_a": "alpha", "model_b": "beta", "winner": "model_a"},
            ...
            {"model_a": "delta", "model_b": "alpha", "winner": "model_b"},
        ]
    )
    model = BradleyTerryModel.from_dataset(BattleDataset.from_dataframe(df)).fit()
    objective = PlayerUncertaintyObjective("alpha")
    initial_report = compute_influence(
        model,
        objective,
        action="add",
    ...
    result = run_ci_reduction_experiment(
        model,
        target_player="alpha",
        policy="influence",
        budget=3,
    ...
    expected = initial_report.head(3).reset_index(drop=True)
    selected = result.selected_actions.reset_index(drop=True)

    assert selected["model_a"].tolist() == expected["model_a"].tolist()
    assert selected["model_b"].tolist() == expected["model_b"].tolist()
    np.testing.assert_allclose(selected["influence"].to_numpy(), expected["influence"].to_numpy())
    assert result.history["step"].tolist() == [0, 1, 2, 3]
