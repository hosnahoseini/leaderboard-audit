...
def _paired_action_mask(frame: pd.DataFrame, selected_matches: pd.DataFrame) -> np.ndarray:
    ...
    base_mask = frame["row_uid"].astype(int).isin(chosen_uids).to_numpy()
    if not {"match_id", "match_copy"}.issubset(frame.columns):
        return base_mask

    chosen_match_ids = set(frame.loc[base_mask, "match_id"].astype(int).tolist())
...
def refit_after_action_subset(
    ...
    if action in {"drop", "flip"}:
        ...
        for _, row in selected_matches.reset_index(drop=True).iterrows():
